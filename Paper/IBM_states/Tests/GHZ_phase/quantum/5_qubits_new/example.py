...
lamb = -1
mu = 0
state_type = "GHZ_phase"
qubit_no = 5

name = state_type
state_vector = []
"""
for i in range(2**qubit_no):
    if i == 0:
        state_vector.append(1/np.sqrt(2))
    elif i == 2**qubit_no-1:
        state_vector.append(1/np.sqrt(2)*1j)
    else:
        state_vector.append(0)
"""

print(np.sum(np.array([item**2 for item in state_vector])))
...
tomography_state = state.IBMstate(IBMtoken, {"qc_type": state_type, "qubit_no": qubit_no, "init_state": state_vector}, "quantum")
...
fidelity_threshold = 0.85 # threshold value of the fidelity to decide when to stop the learning for the IBM states
...
seed_no =  6  # number of RBMs for the active learning
n_samples = 200 # number of measurements in the reference basis
    ...
    for RBM_no in range(len(active_lrng.RBMseeds)):
        if tomography_state.state_type == "MPSstate":
            ...
            if active_lrng.callbacks["rescaled_fidelity"][RBM_no][-1] >= fidelity_threshold:
                s += 1
    if s >= int(len(active_lrng.RBMseeds)/2) + 2:
