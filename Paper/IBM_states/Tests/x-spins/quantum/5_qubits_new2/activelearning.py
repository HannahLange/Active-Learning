...
from qucumber.observables import ObservableBase, SigmaX
...
class MySigmaX_in_X_Basis(MySigmaX):
...
class MyCorr_in_X_Basis(MySigmaX):
...
class MyCorr_in_Y_Basis(MySigmaX):
    """ Class for calculating the correlator observable. Will be used for the callbacks."""
    def __init__(self):
        self.name = "Corr_in_Y"
        self.symbol = "C_in_Y"
...
class Activelearning:
    def __init__(self, tomography_state, folder_name, learning_rate, contrastive_div_steps, num_of_RBMs):
        ...
        RBMseeds = random.sample(range(0, 10000), num_of_RBMs)
        self.RBMseeds = []
        for s in RBMseeds:
            self.RBMseeds.append(str(s))
        ...
        if self.state.state_type == "MPSstate":
            if self.state.qc_type == "LatticeGaugeModel":
                ...
                self.callbacks = {"KL": {}, "KL(Qucumber)": {}, "rescaled_fidelity": {}, "total_density": {}, "density_difference": {}, "correlator_difference": {}}
        else:
    ...
    def sample_first_time(self, n_samples, n_samples_query, with_rotation=True):
        ...
        if self.state.state_type == "MPSstate":
            sxsx, density, total_density, sigmax, correlator = self.state.get_observables()
    ...
    def _rotate_basis(self):
        ...
        for basis in ["Z","X","Y"]:
            ...
            if self.state.state_type == "MPSstate" or self.state.state_type =="IBMstate":
                v_psis = 0
                ...
                fid = 0
                for rbm in list(self.callbacks["rescaled_fidelity"].keys()):
                    fid += self.callbacks["rescaled_fidelity"][rbm][-1]/len(list(self.callbacks["rescaled_fidelity"].keys()))
        ...
        if self.state.state_type == "MPSstate" or self.state.state_type =="IBMstate":
            ...
            best_basis = np.where(np.array(all_fidelities) == max(all_fidelities))[0][0]
            print(best_basis)
        ...
        print(self.state.bases)
        print(self.state.training_bases)
        self.state.save_all(self.folder_name + "/withRotation/" + str(self.n_samples_query) + "samples" + "/" \
                          + str(self.n_samples) + "samples")

    def rotate_MPS_state(self, basis):
        for i in range(self.state.qubit_no):
            if basis == "X":
                rotation = [1j*self.state.lattice.get("sy",i)]
            elif basis == "Y":
                rotation = [-1j*self.state.lattice.get("sx",i)]
    ...
    def _measure_rotated_state(self,basis):
        ...
        # save everything temporary
        name = self.folder_name + "/withRotation/" + str(self.n_samples_query) + "samples" + "/" + str(self.n_samples) + "samples/" #+basis+"_basis"
        self.state.save_all(name)
        self.state.save_all("tmp")
        bases_path = "Training_data/" + name + "/bases.txt"
        train_bases_path = "Training_data/" +name + "/train_bases.txt"
    ...
    def RBM_procedure(self, epochs, period, query_no, path_from_before=None):
        ...
        def get_density(RBM_state):
            if self.best_basis == "X":
                statistics = MySigmaX_in_X_Basis().density_statistics(RBM_state, 1000)
            else:
                statistics = MySigmaX().density_statistics(RBM_state, 1000)
        ...
        for i, seed in enumerate(self.RBMseeds):
            ...
            if self.state.state_type == "MPSstate":
                if self.state.qc_type == "LatticeGaugeModel":
                    ...
                    callbacks = {"KL": KL, "KL(Qucumber)": ts.KL, "rescaled_fidelity": get_fidelity, "total_density": get_total_density,"density_difference": get_density_distance, "correlator_difference": get_corr}
            else:
            ...
            if self.state.state_type == "IBMstate":
                ...
                if self.best_basis == "Y":
                    for i in range(self.state.qubit_no):
                        self.state.qc.h(i)
                        self.state.qc.sdg(i)
                #if self.best_basis == "pi/5 around X":
                #    for i in range(self.state.qubit_no):
                #        self.state.qc.rx(2 * pi - pi / 5, i)
                self.rotated_target_state = self.state._get_state()
