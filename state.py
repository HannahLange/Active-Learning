...
class IBMstate:
    def __init__(self, IBMtoken, qc_dictionary, simulator_type="classical"):
        ...
        self.states = ["initialize", "1", "0", "GHZ", "BellSinglet", "BellTriplet", "W", "Heisenberg"]
...
class MPSstate(IBMstate):
    def __init__(self, qc_dictionary, delta):
        ...
        if self.qc_type == "HeisenbergModel":
            self.lattice = self.gen_heisenberg(delta)
            rnd_mps = p.mp.genSampledState(self.lattice)
            opt_states = self.dmrg(rnd_mps)
            self.qc = opt_states[-1]
    ...
    def gen_heisenberg(self, delta):
        ...
        ham = []
        for i in range(0, self.qubit_no-1):
            ham.append(J/2 * (lat.get("sp",i)*lat.get("sm",i+1) + lat.get("sm",i)*lat.get("sp",i+1)) )
            ham.append((J+delta)  *  lat.get("sz",i)*lat.get("sz",i+1))
            #ham += [-1.05*J*lat.get("sx",i) * lat.get("sx", i+1)]
            #ham += [-1.05*J*lat.get("sy",i) * lat.get("sy", i+1)]
            #ham += [-J*lat.get("sz",i) * lat.get("sz", i+1)]
        ham = p.mp.addLog(ham)
        lat.add("H","H",ham)
        return lat
    ...
    def get_observables(self):
        ...
        for i in range(self.qubit_no-1):
            ...
            self.sxsx.append(sxsx_i)
            if self.qc_type == "HeisenbergModel":
                sysy_i = p.mp.expectation(self.qc, self.lattice.get("sy", i) * self.lattice.get("sy", i+1))
                self.sysy.append(sysy_i)
                szsz_i = p.mp.expectation(self.qc, self.lattice.get("sz", i) * self.lattice.get("sz", i+1))
                self.szsz.append(szsz_i)
                sz = p.mp.expectation(self.qc, self.lattice.get("sz", i))
                density = 0.5+sz
            else:
        ...
        return self.sxsx, self.sysy, self.szsz, self.density, self.total_density, self.sigmax, self.correlator
    ...
    def save_all(self, folder_name):
        ...
        if self.calc_observables and self.qc_type != "GHZ":
            print(np.sum(self.sxsx))
            print(np.sum(self.sysy))
            print(np.sum(self.szsz))
            #print(self.sysy[int((self.qubit_no-1)/2)])
            #print(self.szsz[int((self.qubit_no-1)/2)])
            ...
            obs_path = self.folder_name + "/target_sxsx.txt"
            np.savetxt(obs_path, np.real(self.sxsx), delimiter = ",")
            obs_path = self.folder_name + "/target_sysy.txt"
            np.savetxt(obs_path, np.real(self.sysy), delimiter = ",")
            obs_path = self.folder_name + "/target_szsz.txt"
            np.savetxt(obs_path, np.real(self.szsz), delimiter = ",")
